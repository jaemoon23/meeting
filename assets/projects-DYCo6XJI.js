import{i as me,D as ue,g as ge,f as pe,h as ve,j as ye,k as Ee,l as fe,m as he,L as Be}from"./permission-service-DHxnA_q4.js";import{s as ke,h as U}from"./helpers-F6ttY5H5.js";import{_ as Y,i as De,j as Ie,k as we,g as h,l as Le,m as $e,n as J,b as C,a as T,u as K,o as be,s as Me,p as Se,q as je,t as xe,v as Ce,w as Te,f as Pe,r as Ae,c as qe,d as _e,e as He}from"./project-service-BqVoItuz.js";let g=null,f="overview",R="all",N=null,O=null,z=null,B="month";document.getElementById("loadingOverlay");const Q=document.getElementById("authContainer"),X=document.getElementById("appContainer");function Re(){Q.style.display="flex",X.classList.add("app-hidden")}function Ne(e){Q.style.display="none",X.classList.remove("app-hidden"),document.getElementById("userAvatar").src=e.photoURL||"",document.getElementById("userName").textContent=e.displayName||e.email;const n=document.getElementById("adminBtn");n&&(n.style.display=he()?"inline-flex":"none")}function ee(e){const n=document.getElementById("projectsGrid");let a=e;if(R==="active"?a=e.filter(o=>o.status==="active"):R==="completed"&&(a=e.filter(o=>o.status==="completed")),a.length===0){n.innerHTML=`
    ...
    `,document.getElementById("projectDescription").textContent=e.description||"설명이 없습니다."}function j(){const e=h(g);if(!e)return;const n=C(),a=T(),o=document.getElementById("ganttChart"),s=["blue","purple","green","orange","pink","cyan"];if(n.length===0&&a.length===0){o.innerHTML=`
    ...
    `,k="";n.forEach((i,p)=>{const E=a.filter(v=>v.milestoneId===i.id),I=s[p%s.length],P=E.filter(v=>v.status==="completed").length,A=E.length>0?Math.round(P/E.length*100):0;y+=`
        ...
        `,E.forEach(v=>{const L=v.status==="completed"?"done":v.status==="in_progress"?"progress":"pending";y+=`
                <div class="gantt-task-item" data-task="${v.id}">
                    <span class="gantt-task-status ${L}"></span>
                    ${v.title}
                </div>
            `}),y+="</div>";const w=i.startDate?new Date(i.startDate):t,q=i.endDate?new Date(i.endDate):c,_=Math.max(0,(w-t)/(1e3*60*60*24)),ae=Math.max(1,(q-w)/(1e3*60*60*24)+1),F=_/d*(m.length*r),G=ae/d*(m.length*r);k+=`
            <div class="gantt-timeline-row group-row">
                ...
                <div class="gantt-bar ${I}" style="left: ${F}px; width: ${Math.max(80,G)}px;">
                    ...
                    <div class="gantt-progress-track">
                        <div class="gantt-progress-fill" style="width: ${A}%;"></div>
                    </div>
                </div>
                ${i.endDate?`<div class="gantt-milestone-marker" style="left: ${F+G+8}px;" title="${i.title} 완료"></div>`:""}
            </div>
        `,E.forEach(v=>{const L=v.startDate?new Date(v.startDate):w,oe=v.endDate?new Date(v.endDate):L,le=Math.max(0,(L-t)/(1e3*60*60*24)),ie=Math.max(1,(oe-L)/(1e3*60*60*24)+1),de=le/d*(m.length*r),ce=ie/d*(m.length*r),re=v.status==="completed"?100:v.status==="in_progress"?50:0;k+=`
        ...
        `,D.forEach(p=>{const E=p.startDate?new Date(p.startDate):t,I=p.endDate?new Date(p.endDate):E,P=Math.max(0,(E-t)/(1e3*60*60*24)),A=Math.max(1,(I-E)/(1e3*60*60*24)+1),w=P/d*(m.length*r),q=A/d*(m.length*r),_=p.status==="completed"?100:p.status==="in_progress"?50:0;k+=`
                <div class="gantt-timeline-row" data-task="${p.id}">
                    ${m.map(()=>'<div class="gantt-timeline-cell"></div>').join("")}
                    <div class="gantt-bar ${i} task-bar" style="left: ${w}px; width: ${Math.max(60,q)}px;">
                        <div class="gantt-progress-track">
                            <div class="gantt-progress-fill" style="width: ${_}%;"></div>
                        </div>
                    </div>
                </div>
            `})}y+="</div>";const $=new Date;$.setHours(0,0,0,0);let W="";$>=t&&$<=c&&(W=`<div class="gantt-today-line" style="left: ${($-t)/864e5/d*(m.length*r)}px;"></div>`);const V=new Set;n.forEach((i,p)=>V.add(s[p%s.length])),D.length>0&&V.add("orange");let b='<div class="gantt-legend">';n.forEach((i,p)=>{const E=s[p%s.length];b+=`
    ...
    `,o.querySelectorAll(".zoom-btn").forEach(i=>{i.addEventListener("click",()=>{B=i.dataset.zoom,j()})}),o.querySelectorAll(".gantt-group-title").forEach(i=>{i.addEventListener("click",()=>{i.closest(".gantt-task-group").classList.toggle("expanded")})}),o.querySelectorAll(".gantt-add-milestone-btn").forEach(i=>{i.addEventListener("click",()=>Oe())})}function x(){const e=T(),n=C(),a=document.getElementById("tasksBody"),o=document.getElementById("taskMilestoneFilter");o.innerHTML=`
    ...
    `;const s=o.value;let l=e;if(s==="none"?l=e.filter(t=>!t.milestoneId):s!=="all"&&(l=e.filter(t=>t.milestoneId===s)),l.length===0){a.innerHTML='<div class="tasks-empty">태스크가 없습니다.</div>';return}a.innerHTML=l.map(t=>{var d,r;const c=t.status==="completed"?"completed":t.status==="in_progress"?"in-progress":"pending";t.status==="completed"||t.status;const m=t.priority||"medium",u=t.priority==="high"?"높음":t.priority==="low"?"낮음":"보통";return`
        ...
        `}).join(""),a.querySelectorAll(".status-select").forEach(t=>{t.addEventListener("change",async c=>{const m=c.target.dataset.id;await K(g,m,{status:c.target.value})})}),a.querySelectorAll(".edit-task-btn").forEach(t=>{t.addEventListener("click",()=>se(t.dataset.id))}),a.querySelectorAll(".delete-task-btn").forEach(t=>{t.addEventListener("click",async()=>{confirm("태스크를 삭제하시겠습니까?")&&await be(g,t.dataset.id)})})}function ne(){const e=h(g);if(!e)return;const n=e.members||[],a=ge(),o=document.getElementById("membersList");if(n.length===0){o.innerHTML='<div class="members-empty">팀원이 없습니다.</div>';return}o.innerHTML=n.map(s=>{const l=s.role==="owner",t=s.uid===(a==null?void 0:a.uid)||s.email===(a==null?void 0:a.email),c=s.role==="owner"?"소유자":s.role==="member"?"멤버":"뷰어";return`
